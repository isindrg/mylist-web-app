...
CORS(app, resources={r"/*": {"origins": ["http://listweb-frontend.s3-website.eu-north-1.amazonaws.com", "http://localhost:4200"]}})
...
def update_task(task_id):
    ...
    # Validate input data
    required_fields = ['entity_name', 'task_type', 'time_of_task', 'contact_person', 'status']
    missing_fields = [field for field in required_fields if field not in data or not data[field]]
    if missing_fields:
        return jsonify({'message': f'Missing required fields: {", ".join(missing_fields)}'}), 400
    ...
    task.time_of_task = time_of_task
